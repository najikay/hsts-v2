...
- **F1.2** [T-1] After login, the UI presents a role-appropriate shell: menu items, home dashboard, and permissions all derive from role + course relations.
  - *Amended 2026-08-30 (live session, U-41, lead's ruling):* "role + course relations" is now read literally by the rail, which until this date derived from the role alone. A **COORDINATOR with no taught courses** (`rina.barak`, who coordinates Mathematics and has zero `course_teachers` rows) gets **Dashboard, Question Bank, Approvals, Settings** — Question Bank because the bank's read scope is her whole coordinated subject (BANK contract §7.3), the other six teaching items dropped because each is scoped to courses she teaches and could only ever open an empty screen. A **COORDINATOR who also teaches** (`michal.sharon`) keeps the full rail. TEACHER, STUDENT and PRINCIPAL are unchanged. Her dashboard drops the "Your courses" card on the same rule and keeps the approval cards. **The rail narrows; the routes do not** — every teaching route stays registered for both shapes of coordinator, so deep links, Back targets and notifications still work. The rail was never a permission boundary (ARCHITECTURE §3).
...
- **F4.3** [X] A coordinator MAY approve her own exams — allowed, but the approval log records self-approval (owner: E8, ApprovalService logs it; acceptance case 4.6). Dual-hat coordinator gets no special UX or demo time (non-goal, lead decision 2026-08-19): the coordinator rail is simply the teacher rail + Approvals.
  - *Corrected 2026-08-30 (live session, U-41):* that last clause now holds for the **dual-hat** coordinator only. The dual-hat case is still the one that gets no special UX — it is the *pure* coordinator whose rail narrowed, per F1.2's amendment. Self-approval and its logging are untouched.
...
- **F6.1** [T-6.1–3] Student flow: enter code → enter ID (ת"ז, validated against her own identity) → exam form renders (general text, questions, single-choice answers, optional images).
  - *Amended 2026-08-28 (manual round 1, lead's ruling):* when the student arrives from the dashboard's exam-code card, the code step is a **confirmation**, not a second prompt: the code she already typed is shown read-only with one "Confirm and continue" action and a "Use a different code" link. The join still happens on this screen, so F6.4's one-screen guard is unchanged; what changed is that she is never asked twice for the same four characters.
...
- **F7.1b** [X, decided 2026-08-21] Attention events: when a student's exam window loses OS focus during a live attempt, the client reports it and the teacher's monitor row shows a calm flag (count + timestamps + total time away). Signal, not verdict: benign causes exist, the teacher judges, there is no auto-penalty and the student sees nothing. Honest limit stated for the defense: detection runs on the student's machine, so this is a deterrent and a visibility aid, not a control (same framing as discovery fingerprints). Debounced so a focus flicker is one event.
...
- **F9.4** [T-12, S-37] Report engine: avg/median/decile distribution compared across — executions of the same teacher / same course / same student. Built as one **parameterized report mechanism** (dimension = Strategy) so a new report type is a new strategy class + menu entry, nothing else — that's our answer to "minimal development for new reports".
  - *Amended 2026-09-02 (final round, U-90 full form, team decision ratified by the lead):* the by-student dimension is a **dedicated student report**, not a third copy of the generic comparison. The earlier ruling that a personal score has no field on the principal's wire is reversed on the record (it over-applied S-26, which restricts students; 7.3.1 grants the principal all results and 7.3.2.3 asks for the student's own trajectory). The page shows: a hero with the student's identity and her average; her cards (sittings graded, average, pass rate, best and toughest sitting, average versus her classes); a **score trail** of her approved scores over time in which an unapproved sitting breaks the line rather than being bridged; and "Her sittings" with her score and a versus-class delta beside each sitting's class statistics, plus her marker inside each histogram. Unpublished grades remain invisible here exactly as they are to the student. Wire: REPORTS contract A2 (`ReportRow.subjectScore`, by-student only).
