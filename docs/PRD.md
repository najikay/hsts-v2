...
- **F6.4** [T-6 note] When time expires the server **force-submits** whatever is saved, marks the attempt `TIMED_OUT`, and the client shows a clear "Time is up — your answers were submitted" screen (v1 bug: exam stayed open). No further answer changes accepted server-side regardless of client state.
