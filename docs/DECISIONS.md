...
## ADR-020 — The study bot's corpus carries bank questions WITHOUT the answer key
**Context.** S-28 permits the course question bank as bot source material, and the spec asks for "the questions from the question bank". The bank row also holds `correct_answer`. Two readings were possible: give the model the whole question so it can mark practice attempts, or give it the question and its four options only.
**Decision (lead, E16).** The bot's bank read returns the stem and the four answers **with no marking of which is correct**. `BotBankQuestion` has nowhere to put a key and `QuestionRepository.findBankForBot` does not select the column, so it never leaves the database on that path. A study bot that recites answer keys for material that may be on next week's paper defeats the point of having one, and the spec asks for the questions rather than for the answers.
**Consequences.** The read needs none of `CorrectnessLeakGuardTest`'s sanctioned suffixes precisely because it carries no key — the guard test verifies that rather than assuming it. The bot can explain a practice question and cannot hand out its answer; a student who wants the key still gets it the sanctioned way, from her own checked form after approval (E13.2). Cost: the bot cannot say "you picked the right one" about a bank question, which nobody asked for.

## ADR-021 — The C-4 lockout sentence names the event, not a wall-clock time
**Context.** C-4 requires the same-course lockout message to explain why the bot is locked and when it unlocks. The obvious "unlocks at 10:30" needs a deadline, and the only one the bot feature can reach is the one captured in `ActiveAttempt` when the attempt started. A teacher granting extra time (F7.1, S-20) moves the real deadline without moving that copy.
**Decision (E16).** The sentence names the exam and says the lock lifts when she hands that exam in. No clock. `ActiveAttempt` is left carrying `startedAt` and no deadline.
**Consequences.** The message is true at every moment of the attempt, extension or not, and a student never plans around a time that has quietly moved. The alternative — a deadline on `ActiveAttempt` plus an `ExtendService` hook to refresh the registry on every extension — is an E10/E11 change for a cosmetic gain, and a stale clock on that screen would be worse than no clock (PRD §4.1 forbids mystery states, and "it said 10:30 and it is 10:31" is one). Revisit if the lead wants the countdown: the work is two lines in `AttemptService` and one wire in `HSTSServer`, not a redesign.

## ADR-022 — Bot answers are revealed whole, with an entrance, rather than streamed
**Context.** F12.5 asks for "streaming-style incremental display". Our own wire is a request/response envelope over OCSF with no token channel; both providers can stream, but reassembling a token stream across `Message` would need a new push verb, ordering guarantees and a partial-answer state on the client.
**Decision (E16).** Do not stream. The provider call is non-streaming, and the client shows a typing indicator while it waits and then reveals the whole answer with a 180ms fade-and-rise (`ChatBubble`).
**Consequences.** The perceived difference is a fraction of a second; the difference in moving parts is a protocol. The typing indicator is what satisfies NFR-21's "every async operation shows progress", and it is a real animation with a real stop on every exit path rather than a spinner nobody turns off. If streaming is ever wanted, it is a new push verb and a partial-bubble state, not a change to the provider adapters.
