...
| NFR-20 | Reuse and named design patterns: `DECISIONS.md` + the PLAN §2 table, **named in Javadoc where used** | Javadoc names Strategy (10 files), Builder (26), Singleton (4), Adapter (3), Template Method (2), Command (2), State (1) | Nothing asserts the claim; case 20.1/20.2 is the check | 20.1, 20.2, 20.3 | **PARTIAL — two-unnamed** *(id uncited)* |
...
| NFR-20 | PARTIAL — two-unnamed | Case 20.2 asks that every pattern PLAN §2 claims be named in Javadoc. **Observer** and **DAO/Repository** are not — `ClientEventBus` says "publish/subscribe" and the `server.db.repos` package names no pattern. Two Javadoc lines | L |
