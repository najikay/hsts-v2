...
| F2.4 | Browse/filter by course/topic/difficulty/text; list+detail; lazy image preview | `server.features.bank.BankBrowseService`, `QuestionImages`; `client.features.bank.BankView` | `BankBrowseContract`, `QuestionImagesTest`, `BankSessionTest`, `BankScreenInteractionTest` | 2.6 | **LIVE-unwalked** (the seed ships ten real image bytes since #55, 2026-08-27 — B-8 fixed; the lazy preview is walkable at the manual round) |
...
| F8.2 | Teacher reviews per-student results, approves singly and in bulk, may comment to the student | `GradeApprovalService`, `GradingQueueService`, `OverrideService`, `GradeReviewService`; `client.features.grading.GradingQueueView`, **`GradeReviewView`** over `GradeReviewSession` (route `grading.review`), with `OverrideDialog` and `client.features.results.MarkedPaper` shared with the student's checked form | `GradeApprovalServiceTest`, `GradingQueueServiceTest`, `GradingHandlersTest`, `GradingQueueSessionTest`, `TeacherCommentFlowContract`, **`GradeReviewSessionTest`**, **`GradingInteractionTest`** (Review → the paper with the key → Approve → the row follows; and, since U-46, two ticked rows → one `GRADES_APPROVE` carrying both ids, plus the whole click sequence), `GradeReviewServiceTest` | 8.5 ✅ | **LIVE** *(the review screen landed 2026-08-30, live session, U-38; the id is now cited in `GradeReviewView`, `GradeReviewSession`, `MarkedPaper` and `Routes.GRADE_REVIEW`. **In bulk** became true the same day, U-46: the approve reads a tick column rather than the table's row selection, which had made every click replace what came before it)* |
...
| F9.3 | Principal browses bank, exams and results with **zero** mutating verbs authorized, and every row she browses **opens** (U-44, 2026-08-30) | `server.features.reports.DataBrowseService`, `server.features.bank.BankReadHandlers`, `server.features.approval.ApprovalService.preview` (APPROVAL amendment A1), `server.core.Authorization`; `client.features.data.DataView` plus `DataQuestionView` / `DataExamView` / `DataSittingView` over three read-only sessions | `DataBrowseServiceTest`, `AuthorizationTest`, `BankReadHandlersTest`, `ApprovalServiceTest$Preview` (a principal admitted, a student refused, no decision verb reachable), `DataDetailSessionTest`, `DataDetailCopyTest`, `DataBrowserInteractionTest` (asserts no push button on the list, and no control wearing a mutating label on any of the three details), `SessionRoutesTest`, `RoleNavTest` | 11.1–11.4 | **LIVE-unwalked** |
...
| NFR-17 | Test data prepared in the DB by a versioned Java loader, idempotent, one command | `server.db.seed.SeedLoader` / `SeedMain` / `SeedDocument`, console button | `SeedLoadedDbContract`, `SeedDocumentTest`, `SeedDatasetContract` | 17.1, 17.2, 17.3 | **LIVE-unwalked** (B-8 fixed 2026-08-27, #55 — ten images loaded from classpath resources, missing-image fails the load by name) |
...
| NFR-17 | ~~PARTIAL — images-missing~~ **resolved 2026-08-27 (#55)** | Ten seeded questions carry real bytes drawn by a committed generator; a missing image fails the seed load by name. E2.16's seed review pass remains the open half (lead's eyes on the rendered screens). | A (done), L (review open) |
