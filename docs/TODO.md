...
- [x] E7.11 Builder — metadata step (name, duration, texts with student/teacher tabs) *(built in #52; **ticked now rather than then**, on E6.9's and E7.10's precedent: the builder was on no rail until assembly 3 registered `exams.build`, so a reviewer could have disproved the tick. The student/teacher texts are a segmented control as of PR24, not a `TabPane` - it was the only one in the client and E7.13 put a second strip on the same screen)*
- [x] E7.12 Builder — manual tab: bank picker (filter/search), selected list with points editors + reorder, live Σ/100 indicator (green at 100) *(2026-08-26: the selected list, points editors, reorder and live Σ landed in #52; **the picker's add path is what completes it**, and it was unbuildable until BANK amendment A1 put `latestVersionId` on `BankQuestionRow` - `QuestionPin` keys on a version id and the bank wire carried none. The row is pinned at the version she was shown, which is what makes E7.7's badge a drift detector afterwards rather than a coincidence. **T-3.9 is refused on the click**, on `displayId5` and never on a version id, so 11005 v1 beside 11005 v2 is one question here as it is to `uq_exam_version_questions_question`. Filtered client-side over the whole course bank, which is one load rather than a round trip per keystroke; the picker lives inside `ExamBuilderView` because a separate thin FX class would need a JaCoCo exclusion and the pom is not Member A's. Four distinct empty states, because an empty bank, a filter matching nothing, a failed load and a truncated one are four different things to be told)*
- [x] E7.13 Builder — auto tab: criteria form (topic rows, difficulty sliders), generate → editable result, infeasibility report rendering ⚑ *(2026-08-26, **ruled Member A's** on the lead's reversal of the Aug-25 scope call: the auto tab is a mode of the screen its author owns. A segmented switch beside the manual tab, a criteria grid whose first row is always the course-wide quota, and counts rather than sliders - `TopicQuota` carries integers and a slider would be a control that cannot express what the wire does. **The live rule is `ExamValidator.quotaProblem` itself**, the method the handler refuses with, exactly as the Σ indicator is `pointsProblem`: §7.3a's refusal has to name both legal shapes and `ExamBuildMessages` owns that wording, so this client composes none of it (ruling 4). A feasible answer **replaces the paper and lands her on it**, which is what makes F3.3's "editable before save" true rather than claimed - there is no second composition anywhere in the session. An infeasible one changes nothing at all: §7.2's report is the useful outcome and a refusal must not cost her the paper she had. **The infeasibility sentence is composed once, in `ExamBuildCopy`**, per ruling 4, and PRD F3.3's own string `Topic 'Algebra': requested 5 Hard, bank has 2` is pinned by a copy test alongside §7.1's other three shapes. Where §7.1's table and the PRD disagree on quoting, the PRD wins: it is the acceptance artefact)*
- [x] E7.14 Version history + "question has newer version" badges with update-question action *(2026-08-26. **Attribution stated so it can be disagreed with rather than assumed:** the *version history* half is E7.10's exam list, whose master/detail already lists every version of an exam with its state, question count and duration; nothing new was built for it here. The *badges* have rendered since #52 off `pinnedVersionNo` vs `latestVersionNo`. What PR24 adds is the **update action**, and it was unbuildable until EXAM_BUILDER §4 amendment A1: the two version numbers are enough to draw the badge and not enough to press it, because `QuestionPin` keys on an id. The whole vertical is Member A's under the lead's opening of 2026-08-26 - DTO component, the dated amendment, the repository subquery, the service population, the DTO shape rows. **The ruling said the id "rides the same subquery" as the number and it does not:** `max(versionNo)` yields a number and the id of the row holding it is a second question; `max(id)` is a valid query that passes 37 of the 38 repository cases and re-pins to the wrong version of a question whenever ids and version numbers were not allocated in the same order. `latestIdIsTheIdAtTheLatestNumber` tells them apart on both engines. Re-pinning is a change to the list like a reorder, so it rides `EXAM_VERSION_SAVE` and takes no verb of its own. **The row keeps the old wording until she saves and says so**, because the wire carries the newer version's id and none of its text)*
...
- [x] E7.16 Session tests + integration test (manual + auto + infeasible + Σ≠100 + versioning) *(2026-08-26: `ExamBuilderSessionTest` covers the screen FX-free across ten nested groups, and `ExamBuildRoundTripIntegrationTest` walks all five journeys the row names through the **assembled** stack against real MySQL. **MySQL only, and that is not laziness:** the H2 schema is generated from the entities and reproduces no foreign keys and no CHECK constraints, and three of the five journeys lean on one - `ck_evq_points`, `uq_exam_version_questions_question`, `uq_exam_versions_no`. Nothing is asserted against what a write returned; every check is a second call in a new transaction, so it sees committed state rather than the first-level cache. Questions are seeded and exams are not, because the bank is the input here and its own write path is proven by `BankRoundTripIntegrationTest`. **A plant found a hole in this file rather than in the code:** writing the carried composition onto the predecessor left version 1 looking untouched, since it already held those rows, and version 2 empty, and every assertion still passed. The missing assertion - what version 2 actually carries - is now there)*
- [ ] E7.17 Acceptance pass vs T-3 with [L] ⚑ — *deliberately not ticked, and not Member A's to tick. The lead's ruling of 2026-08-26: scenario 3's **below-screen half is discharged by the acceptance campaign** (all nine cases passed; 3.4 failed as written and its own text was rewritten, B-9), and the **on-screen half rides the next manual round** now that the builder is reachable. Member A owes no separate pass. The row stays open because the round has not happened*
...
- [x] E12.6 Per-student review screen: checked form view (correct/wrong marks), override dialog (score+reason), comment box, approve — *server side was always done: `GradeReviewService` assembles the marked paper and `GRADE_REVIEW_GET` serves it to the owning teacher, with the ticks re-run through `AutoGrader` so a paper's marks and the score above them can never come from two different rules. **The comment box landed 2026-08-23** in the override dialog on the queue screen: two boxes, separated, one labelled for the record and one saying the student will read it. **The screen landed 2026-08-30** (live session, U-38): `GradeReviewView` over `GradeReviewSession` on route `grading.review`, opened by a per-row Review action on the queue, carrying Approve and Change score on the paper itself. Its question cards are `MarkedPaper`, lifted out of `CheckedFormView` so the teacher approves the same drawing her student will read, and the override dialog is `OverrideDialog`, now shared with the queue rather than copied*
...
- [x] E16.9 Bot management service: create bot (one per course — second teacher joins existing, S-30), sources CRUD (edit-locked), active toggle, co-teacher notifications — *the **U** of that CRUD was ticked without existing until 2026-08-26: acceptance case 13.4 found add and remove and no update anywhere in the stack (**B-21**). `BOT_SOURCE_UPDATE` landed in batch B — the row keeps its id, its author and its place, its domain version is bumped, and the consult on `EntityRef.BOT_SOURCE` runs after the scope check per E6.14 with a refusal that names the holder. `BOT_WIRE_CONTRACT` A1. **Now true as written.** And the **D** arrived on 2026-08-30 (live session, **U-39**): `BOT_DELETE` deletes a course's bot and its sources in one transaction, refuses with `CONFLICT` and a count when students have talked to it because those transcripts are their records (S-33, and `bot_sessions` is `RESTRICT` for the same reason), and consults the `EntityRef.BOT_SOURCE` lock over every source before it takes any of them. `BOT_WIRE_CONTRACT` A3.*
