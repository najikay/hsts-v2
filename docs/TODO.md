...
- [ ] E12.5 Grading queue screen: executions awaiting grading, per-execution student table (auto scores, status) — *server half done: `GRADING_QUEUE_GET` and `GRADING_EXECUTION_GET` on the router behind the existing teacher gate, assembled by `GradingQueueService` from reads that already existed. The queue's rules are exclusions — a sitting still running, one nobody has marked, and one already signed off are all absent for three different reasons, because a queue that never empties stops being read. Not ticked: the screen*
...
- [ ] E13.5 Export/print view of the checked form (S-36) — printable layout — *the print toggle is in, on the `TeacherResultsView` pattern: one style class that drops the application chrome and leaves the paper in a single column. Not ticked until the lead has seen it render*
