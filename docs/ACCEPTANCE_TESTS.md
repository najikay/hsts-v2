...
| 8 | Exam checking (בדיקת מבחנים) | 7 | ⚠ 6 passed, 1 awaiting re-walk (8.4, since A3 landed the comment wire) — B-4, B-5 fixed |
| 9 | Viewing an exam grade (צפיה בציון הבחינה) | 5 | ⚠ 3 passed, 1 passed below the screen, 1 not walked (9.3 print) |
...
| 8.4 | Change it again with a justification, and add a comment to the student. Then inspect the stored record. | Original auto grade, the change, and the reason are all stored — a full audit trail (F8.3). The comment is saved (S-22). | **Justification half passed on the 2026-08-23 walk:** with a reason the score changed, the row showed **Adjusted**, and the **Auto column kept the machine’s original** — F8.3’s audit trail, the change visible and the evidence intact. The comment half could not be walked then, because `teacherComment` had no write path at all (B-3). **B-3 is now fixed** — the lead landed GRADING amendment A3, so a comment rides the override. The case is therefore **awaiting a re-walk** rather than blocked; it is not marked passed on the strength of the wire existing, because the point of this row is that somebody saw it work. | ⚠ | B-3 (fixed) |
