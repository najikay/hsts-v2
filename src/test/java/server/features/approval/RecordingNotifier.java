 ...
 * (see its javadoc). Three things in this epic depend on that: an approval and a rejection
 * must reach the exam's <em>author</em> and nobody else, a supersede must reach the
 * <em>coordinator</em> and nobody else, and a coordinator approving her own exam must be
 * told nothing at all, because she is looking at the confirmation.
 *
 * <p>A copy of the one in {@code server.features.exam} rather than a shared class: both are
 * package-private test doubles, and a shared one would have to be public API for tests,
 * which is a heavier commitment than forty lines of fixture.
 */
